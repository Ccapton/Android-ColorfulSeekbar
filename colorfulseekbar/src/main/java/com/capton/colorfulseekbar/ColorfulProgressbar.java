...
public class ColorfulProgressbar extends ViewGroup {
    ...
    private void getParameter(Context context, AttributeSet attrs){
        if(attrs!=null) {
            ...
            style = ta.getString(R.styleable.ColorfulProgressbar_style2);
            if (!STYLE_NORMAL.equals(style) && !STYLE_COLORFUL.equals(style)) {
                style = STYLE_COLORFUL;  //如果没有在xml中显示设置style，默认使用双色进度条
            }
            progress = ta.getInteger(R.styleable.ColorfulProgressbar_progress2, (int)progress);
            secondProgress = ta.getInteger(R.styleable.ColorfulProgressbar_secondProgress2,(int)secondProgress);
            maxProgress = ta.getInteger(R.styleable.ColorfulProgressbar_max2, (int) maxProgress);
            backgroundColor = ta.getColor(R.styleable.ColorfulProgressbar_backgroundColor2, backgroundColor);
            progressColor = ta.getColor(R.styleable.ColorfulProgressbar_progressColor1_2, progressColor);
            progressColor2 = ta.getColor(R.styleable.ColorfulProgressbar_progressColor2_2, progressColor2);
            animationOn = ta.getBoolean(R.styleable.ColorfulProgressbar_animation2,animationOn);
            ta.recycle();
            initSettings();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        * */
        ...
        Log.w("ColorfulProgressBar", "onMeasure getMeasuredWidth: "+getMeasuredWidth());
        Log.w("ColorfulProgressBar", "onMeasure getMeasuredHeight: "+getMeasuredHeight());
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if(!once) {
            ...
            Log.w("ColorfulProgressBar", "childcount: "+getChildCount());
            Log.w("ColorfulProgressBar", "onlayout getMeasuredWidth: "+getMeasuredWidth());
            Log.w("ColorfulProgressBar", "onlayout getMeasuredHeight: "+getMeasuredHeight());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(setBackgroudColor) {
            ...
            setBackgroundResource(R.drawable.background);
            Log.e("ColorfulProgressBar", "onDraw: ");
